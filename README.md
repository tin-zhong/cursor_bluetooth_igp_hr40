...
- 从“开始运动”到“结束运动”持续记录心率采样，运动数据保存在本机 Room 数据库（SQLite）中。
...
```text
dist/hr40-offline-fitness-v3.2.0.apk
```
...
- 训练记录改为仅使用 Room 数据库存储（不再写入 JSON 文件）
- 首次启动自动把历史 JSON 训练记录迁移进数据库
...
如果安装后界面没有变化，请先卸载旧版 App，再安装 `dist/hr40-offline-fitness-v3.2.0.apk`。
