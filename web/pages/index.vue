...
<template>
  <div class="max-w-3xl mx-auto p-4 sm:p-6">
    <AppHeader :title="headerTitle" :subtitle="user?.email || undefined">
      <template #actions>
    ...
    <UCard v-else-if="!workouts.length">
      <p class="text-gray-500">暂无训练记录。请先在 App 中同步数据。</p>
    </UCard>
        ...
        <UCard class="hover:ring-2 hover:ring-primary/20 transition">
          <div class="flex items-center justify-between gap-3">
            <div>
              ...
              <p class="text-sm text-gray-500 mt-1">{{ formatDateTime(workout.start_millis) }}</p>
            </div>
