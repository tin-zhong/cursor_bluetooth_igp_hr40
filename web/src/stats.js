...
function caloriesPerMinute(profile, bpm) {
  let kilojoulesPerMinute;
...
function caloriesForSegment(profile, current, next) {
...
export function estimateCalories(profile, samples, workoutType) {
...
export function calculateWorkoutStats(profile, samples, workoutType) {
  if (!samples.length) {
    return {
      minBpm: 0,
      maxBpm: 0,
      avgBpm: 0,
      calories: 0,
      zoneMillis: new Array(ZONE_LABELS.length).fill(0),
      sampleCount: 0,
  ...
  const zoneMillis = new Array(ZONE_LABELS.length).fill(0);
  for (let i = 0; i < samples.length - 1; i++) {
...
export function formatDuration(startMillis, endMillis) {
...
export function formatZoneDuration(millis) {
